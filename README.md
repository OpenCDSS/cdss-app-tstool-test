    ...
    commands/                               Folder containing command tests.
      CommandName/                          Command name, as in TSTool.
        ...
        Test_CommandName_TestName.TSTool    Individual test command files to run.
                                            ...
                                            Newer tests use the following naming.
        data/                               Data used for command tests, usually small datasets.
        expected-results/                   Expected output from test when files are involved,
                                             generated, inspected, and saved for continued use.
        results/                            Results generated by the current test runs.
        test-CommandName-testname.tstool    Individual test command files to run.
    scripts/                                Useful scripts used in testing.
    test-suites/                            Folder for saving test suites, when the above command
                                             tests are run in groups.
      commands-general/                     Folder for general command test suite, basically to
...
menu and browse to the `test-suites/commands-general/run` folder and select
`RunRegressionTest_commands_general_Windows.TSTool` (or similar test suite).
Once loaded into the TSTool interface, use the ***Run All Commands*** button.
...
In such cases, a `test-suites` folder (or similar) may be created to group tests.

## Troubleshooting ##

The following are common issues:

*   **Can't find command file with debug turned on** - this causes the following command files run in a suite
    to also have debug turned on.  See the `scripts/find-debug-command-files.bash` script to find the command file to fix.
